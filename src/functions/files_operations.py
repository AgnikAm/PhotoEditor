...
def pick_files_open(old_path, new_path, photo_flet, photo_pil, e: ft.FilePickerResultEvent) -> None:
        if e.files:
            file_path = e.files[0].path.replace("\\", "/")
            old_path.value = file_path
            photo_flet.src = file_path
            ...
            photo_pil.value = open_image(new_file_path)

            photo_flet.update()

def pick_file_save(image: Image.Image, e: ft.FilePickerResultEvent) -> None:
    save_location = e.path
    if save_location:
        save_image(image, save_location)
